...
## check motion
log "starting motion monitoring script"
export HASSIP="$(get_config HASSADDRESS)"
export HASSAPIKEY="$(get_config HASSAPIKEY)"
HASSIP="$(get_config HASSADDRESS)" HASSAPIKEY="$(get_config HASSAPIKEY)" CAMERA_NAME="$(get_config HASS_CAMERA_NAME)" FRIENDLY_NAME="$(get_config HASS_FRIENDLY_NAME)" ./check_motion_v2.sh > ./check_motion_v2.log &
log "monitoring motion!"
sleep 5
