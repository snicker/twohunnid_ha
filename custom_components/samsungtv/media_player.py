...
CONF_ENCRYPTED_CONFIG = 'encrypted_config'
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_ENCRYPTED_CONFIG): cv.string,
})
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if config.get(CONF_HOST) is not None:
        ...
        encrypted_config = config.get(CONF_ENCRYPTED_CONFIG)
    ...
    if ip_addr not in known_devices:
        ...
        add_entities([SamsungTVDevice(host, port, name, timeout, mac, uuid, encrypted_config)])
...
class SamsungTVDevice(MediaPlayerDevice):
    ...
    def __init__(self, host, port, name, timeout, mac, uuid, encrypted_config):
        ...
        if self._encrypted_config:
            self._config = SamsungConfig.load(self._encrypted_config)
            self._config.name = 'HomeAssistant'
            self._config.description = name
            self._config.id = 'ha.component.samsung'
        else:
            self._config = {
                'name': 'HomeAssistant',
                'description': name,
